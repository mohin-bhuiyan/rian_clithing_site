...
let cart_data=[];
try{
       cart_data= from_local?JSON.parse(from_local):[];
}catch(e){
       cart_data=[];
}
...
cart_data.forEach(item=>{
        ...
        let gotten_item = data.find(prod=> parseInt(item.id)===parseInt(prod.id));
        // console.log(gotten_item);
         if(!gotten_item){return}
        // dom manipulation....
...
function all_product_settings(){
    ...
    cart_item.forEach(item=>{
        ...
        //ei item er html e stor kora "data-id"
          ...
          let item_size= item.getAttribute('data-size');
         ...
         quantityUp(cart_quant_up,cart_quant_input,item_id,item_size);
        //  down clik event
          quantityDwn(cart_quant_dwn,cart_quant_input,item_id,item_size);
...
// component
function quantityUp(cart_quant_up,cart_quant_input,item_id,item_size){
   ...
   cart_quant_up.addEventListener("click", ()=>{
               ...
               let cart_data= from_local?JSON.parse(from_local):[];
                  ...
                  let gotten_lc_data= cart_data.find(lc_data=> parseInt(lc_data.id)===parseInt(item_id) && lc_data.size==item_size);
                     if(!gotten_lc_data){return};
...
function quantityDwn(cart_quant_dwn,cart_quant_input,item_id,item_size){
    cart_quant_dwn.addEventListener("click", ()=>{
